...
MFSC_ROOT = "/mnt/c/EMG_data/precomputed_mfsc"
...
print("Mean TV (MFSC):     ", tv_mfsc.mean())
print("Mean diff (WST-MFSC):", (tv_wst - tv_mfsc).mean())
print("Fraction WST < MFSC:", np.mean(tv_wst < tv_mfsc))

# -------------------------------------------------
# PLOT
# -------------------------------------------------
plt.figure(figsize=(6,5))
plt.boxplot([tv_mfsc, tv_wst], labels=["MFSC", "WST"], showfliers=False)
plt.ylabel("Temporal Variation (L2)")
plt.title("Temporal Stability: MFSC vs WST")
plt.grid(alpha=0.3)
plt.tight_layout()
plt.show()
